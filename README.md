...
במטלה זו, הלוח יהיה מיוצג ע"י המחלקה 
`Board`.
...
שמות הערים בקוד יהיו בדיוק כמו על המפה, בלי הרווחים. לדוגמה, השם של העיר ניו יורק יהיה
`NewYork`.
...
במחלקה "לוח" יש לממש את השיטות הבאות:

* `set_infection_level` - המקבלת כפרמטר עיר ומספר חיובי,
ומשנה את מספר קוביות-המחלה באותה עיר. לדוגמה:
`board.set_infection_level(City::HongKong, 2)`
...
* אופרטור פלט - מציג את הלוח בפורמט כלשהו לבחירתכם.
...
1. טיפול במחלה - `treat` - הורדת קוביית-מחלה אחת מהעיר שנמצאים בה (הפחתת רמת המחלה ב-1).
   *  אם כבר התגלתה תרופה למחלה, אז פעולת "ריפוי מחלה" מורידה את כל קוביות המחלה מהעיר שנמצאים בה (הפחתת רמת המחלה ל-0).
...
בנוסף, יש לממש את הפונקציות הבאות:

*  `take_card` - לקיחת קלף-עיר כלשהו. פעולה זו מדמה את התהליך שבו השחקן מקבל קלפים מהקופה בתחילת המשחק או במהלכו. יש רק קלף אחד של כל עיר, ולכן אם מבצעים פעולת `take_card` על קלף שכבר נמצא בידי השחקן, לא יהיה כל שינוי במצב השחקן (אין צורך לזרוק חריגה במקרה זה).
*  `role` - פונקציה המחזירה את התפקיד של השחקן (ראו רשימת התפקידים למטה), לצורך תצוגה.


## תפקידים

ישנם תפקידים שונים של שחקנים, שיש להם כישורים מיוחדים (שימו לב: הכישורים לא לגמרי זהים למשחק המקורי):
...
1. רוקח - `Pharmacist`: יכול לבצע פעולת "טיפול במחלה", לא רק בעיר שהוא נמצא בה, אלא בכל עיר - ע"י השלכת קלף של אותה העיר.
1. מהנדסת גנים - `GeneSplicer`: יכולה לבצע פעולת "גילוי תרופה" בעזרת 5 קלפים כלשהם - לא דווקא מהצבע של המחלה.
1. רופא שטח - `FieldDoctor`: יכול לבצע פעולת "טיפול במחלה" לא רק בעיר שהוא נמצא בה אלא בכל עיר סמוכה לעיר שהוא נמצא בה (ע"פ מפת הקשרים).
...
* בדיקות מקיפות לכל הפונקציות הדרושות.
   ...
   * אין צורך לבדוק מצבים שבהם יש כמה שחקנים שונים על אותו לוח.
