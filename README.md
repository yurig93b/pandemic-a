...
* `show` - שיטה ללא פרמטרים, המציגה את לוח-המשחק בפורמט כלשהו לבחירתכם.
...
בנוסף, יש לממש שיטה בשם `take_card` - לקיחת קלף-עיר כלשהו מהקופה.
פעולה זו היא תמיד חוקית.
יש רק קלף אחד של כל עיר, ולכן אם מבצעים פעולת
`take_card`
על קלף שכבר נמצא בידי השחקן, לא יהיה כל שינוי במצב השחקן.
...
ישנם סוגים שונים של שחקנים, שיש להם כישורים מיוחדים (שימו לב: הכישורים לא לגמרי זהים למשחק המקורי):
...
1. מדענית - `Scientist`: יכולה לבצע פעולת "גילוי תרופה" בעזרת 4 קלפים בלבד (במקום 5).
...
1. מנהל שטח - `FieldDirector`: יכול לבצע פעולת "טיפול במחלה" לא רק בעיר שהוא נמצא בה אלא בכל עיר סמוכה לעיר שהוא נמצא בה (ע"פ מפת הקשרים).


בשלב א עליכם לכתוב כותרות ובדיקות-יחידה מקיפות.
אין צורך לבדוק את השיטה
`show` - כיוון שלא הגדרנו את הפורמט שלה.
